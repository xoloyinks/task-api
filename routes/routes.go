...
func SetupRoutes(taskHandler *handlers.TaskHandler, authHandler *handlers.AuthHandler, teamHandler *handlers.TeamHandler, boardHandler *handlers.BoardHandler, sseHandler *sse.SSEHandler) http.Handler {
	r := http.NewServeMux()
	...
	r.HandleFunc("DELETE /teams/{id}/members/{memberID}", utils.Make(middleware.AuthMiddleware(teamHandler.RemoveMember)))
